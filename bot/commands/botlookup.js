...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('botlookup')
        .setDescription('Looks up detailed bot information')
        .addUserOption(option => 
            option.setName('bot')
                .setDescription('The bot to lookup')
                .setRequired(true)),
    ...
    async execute(message, args) {
        ...
        const embed = await createBotLookupEmbed(bot, message.client);
    ...
    async executeSlash(interaction) {
        ...
        const embed = await createBotLookupEmbed(bot, interaction.client);
        await interaction.reply({ embeds: [embed] });
    }
};

async function createBotLookupEmbed(bot, client) {
